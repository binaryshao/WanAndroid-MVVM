...
import com.sbingo.wanandroid_mvvm.ui.fragment.KnowledgeFragment

/**
 * Author: Sbingo666
 * Date:   2019/4/26
 */
class KnowledgeViewPagerAdapter(fm: FragmentManager) : FragmentStatePagerAdapter(fm) {
    ...
    fun setData(data: List<Chapter>) {
        list.run {
            ...
            addAll(data)
            forEach {
                fragments.add(KnowledgeFragment(it.id))
            }
        }
